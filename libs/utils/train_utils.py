...
def make_optimizer(model, optimizer_config):
    ...
    blacklist_weight_modules = (LayerNorm, torch.nn.GroupNorm, torch.nn.LayerNorm, RMSNorm)
    ...
    for mn, m in model.named_modules():
        for pn, p in m.named_parameters():
            ...
            if pn.endswith('bias'):
                ...
                no_decay.add(fpn)
            elif pn.endswith('dt_bias_fwd') or pn.endswith('A_log_fwd') or pn.endswith("D_b_fwd") or pn.endswith("D_fwd") or pn.endswith("A_b_log_fwd") or pn.endswith("forward_embed_fwd") or pn.endswith("backward_embed_fwd"):
                # corner case for mamba
                no_decay.add(fpn)
            elif pn.endswith('dt_bias_bwd') or pn.endswith('A_log_bwd') or pn.endswith("D_b_bwd") or pn.endswith("D_bwd") or pn.endswith("A_b_log_bwd") or pn.endswith("forward_embed_bwd") or pn.endswith("backward_embed_bwd"):
                # corner case for mamba
                no_decay.add(fpn)
    ...
    for iter_idx, video_list in enumerate(train_loader, 0):
        ...
        if (iter_idx != 0) and (iter_idx % print_freq) == 0:
            ...
            for key, value in losses_tracker.items():
                if key != "final_loss":
                    log_dict[f'train/{key}'] = value.val
...
def valid_one_epoch(
    ...
    # --- W&B Validation Loss ---
    # Prefix keys with 'val/' and sync with 'epoch'
    if not is_test and global_step:
        log_dict = {
            'val/epoch': curr_epoch,
            'val/final_loss': losses_tracker['final_loss'].val,
            'global_step': global_step,
        }
        # Add individual losses
        for key, value in losses_tracker.items():
            if key != "final_loss":
                log_dict[f'val/{key}'] = value.val
    ...
    # call the evaluator
    mAP, average_mAP = evaluator.evaluate(results, verbose=True)
    if not is_test and global_step:
        # Construct log dictionary
        map_log_dict = {
            'val/mAP_avg': average_mAP,
            'global_step': global_step
        }
        for tiou, tiou_mAP in zip(evaluator.tiou_thresholds, mAP):
                map_log_dict[f'val/mAP@{tiou:.2f}'] = tiou_mAP

        wandb.log(map_log_dict, step=global_step)
    else:
        wandb.run.summary["test_mAP_avg"] = average_mAP

        for tiou, tiou_mAP in zip(evaluator.tiou_thresholds, mAP):
                wandb.run.summary[f'test_mAP@{tiou:.2f}'] = tiou_mAP

    # # log mAP to tb_writer
    # if tb_writer is not None:
    #     tb_writer.add_scalar('validation/mAP', mAP, curr_epoch)

    print(f"Max score: {results['score'].max() if len(results['score']) > 0 else 'No predictions'}")
