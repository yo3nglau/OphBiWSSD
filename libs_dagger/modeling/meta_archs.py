...
from .blocks import MaskedConv1D, Scale, LayerNorm, MaskMambaBlock
...
@register_meta_arch("OphBiWSSD")
class OphBiWSSD(nn.Module):
    def __init__(
            self,
    ...
    def before_refine_decode_offset(self, out_offsets, pred_start_neighbours, pred_end_neighbours, val_loss_flag=False):
        ...
        if not self.use_trident_head:
            if self.training or val_loss_flag:
                out_offsets_tensor = torch.cat(out_offsets, dim=1)
            else:
                out_offsets_tensor = out_offsets
            return out_offsets_tensor
            ...
            if self.training or val_loss_flag:
                out_offsets_tensor = torch.cat(out_offsets, dim=1)
                out_offsets_tensor = out_offsets_tensor.view(out_offsets_tensor.shape[:2] + (2, -1))
                pred_start_neighbours = torch.cat(pred_start_neighbours, dim=1)
                pred_end_neighbours = torch.cat(pred_end_neighbours, dim=1)

                pred_left_dis = torch.softmax(pred_start_neighbours + out_offsets_tensor[:, :, :1, :], dim=-1)
                pred_right_dis = torch.softmax(pred_end_neighbours + out_offsets_tensor[:, :, 1:, :], dim=-1)

            else:
                out_offsets_tensor = out_offsets.view(out_offsets.shape[0], 2, -1)
                pred_left_dis = torch.softmax(pred_start_neighbours + out_offsets_tensor[None, :, 0, :], dim=-1)
                pred_right_dis = torch.softmax(pred_end_neighbours + out_offsets_tensor[None, :, 1, :], dim=-1)
    ...
    def before_refine_losses(
        ...
        decoded_offsets = self.before_refine_decode_offset(out_offsets, out_start_logits, out_end_logits, val_loss_flag=True)  # bz, stack_T, num_class, 2
    ...
    def decode_offset(self, out_offsets, pred_start_neighbours, pred_end_neighbours,start_offset_list,end_offset_list,points, val_loss_flag=False):
        # decode the offset value from the network output
        # If a normal regression head is used, the offsets is predicted directly in the out_offsets.
        # If the Trident-head is used, the predicted offset is calculated using the value from
        # center offset head (out_offsets), start boundary head (pred_left) and end boundary head (pred_right)
    ...
    def losses(
        ...
        decoded_offsets = self.decode_offset(out_offsets, out_start_logits, out_end_logits,start_offset_list,end_offset_list,points, val_loss_flag=True)  # bz, stack_T, num_class, 2
